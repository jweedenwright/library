...
- General purpose technologies - good for multiple applications
...
- In general, an LLM generates tokens based on the text in the context window, and the probability of all tokens in the vocabulary.
...
- **RLAIF - Reinforcement Learning from AI Stage:** Similar to **RLHF** but input comes from model (i.e. AI)
...
- **Distillation**
  ...
  - Larger LLM _Teacher_ teaches smaller LLM _Student_
    ...
    - The combined sets of loss are then used to update weights of the student model using _back propogation_
...
```
Solve a quesiton answering task with interleaving Thought, Action, Observation steps.
